...
export default function ConfirmPage(){
  ...
  const title = useMemo(()=> serviceId==="svc-01" ? "ถอดPVC และต่อเล็บ" : serviceId==="svc-02" ? "ถอดPVC ทาสีเจล" : "ทาสีเจล", [serviceId]);
  ...
  const handleCreate = async ()=>{
    ...
    try{
      ...
      if(!res.ok){ alert(`สร้างการจองไม่สำเร็จ (${res.status}) : ${(payload as any)?.error || "unknown"}`); return; }
      const bk = payload as { id?: string };
      if(!bk?.id){ alert("สร้างการจองแล้ว แต่ไม่ได้รับรหัสการจอง"); return; }
    ...
    <LayoutWrapper>
      ...
      <div className="rounded-3xl border border-pink-100 shadow-[0_6px_14px_rgba(255,182,193,0.25)] bg-pink-50/60 p-4">
        <div className="flex gap-3">
          <div className="relative w-[96px] h-[96px] rounded-2xl bg-white ring-1 ring-pink-100" />
