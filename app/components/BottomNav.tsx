...
export default function BottomNav(){
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-pink-50/85 bottom-blur border-t border-pink-100">
      <ul className="mx-auto grid max-w-screen-sm grid-cols-5 py-2">
          ...
          return (
            <li key={href} className="text-center">
              <Link href={href} className={active ? "text-pink-600 flex flex-col items-center gap-1":"text-pink-800/70 flex flex-col items-center gap-1"}>
                <Icon className="h-5 w-5"/><span className="text-[11px]">{label}</span>
              </Link>
            </li>
          )
        })}
