    ...
    <LayoutWrapper>
      <section className="px-4 pt-4">
        ...
        <div className="mx-auto max-w-md mb-4 rounded-full bg-pink-50 border border-pink-100 grid grid-cols-3 overflow-hidden">
          {["PENDING","DONE","CANCELLED"].map(k=>(<button key={k} onClick={()=>setTab(k as any)} className={`py-2 text-xs ${tab===k?'bg-pink-200/70 font-semibold':''}`}>{k==="PENDING"?"รอเข้ารับบริการ":k==="DONE"?"เข้ารับบริการแล้ว":"ยกเลิก"}</button>))}
        </div>
        <div className="space-y-3">
          ...
          {list.map(b=>(
            <article key={b.id} className="rounded-2xl border border-pink-100 bg-white p-4 shadow-sm">
              ...
              <div className="flex gap-2">{tab==="PENDING"&&<button onClick={()=>markDone(b.id)} className="px-3 py-1.5 rounded-lg bg-pink-500 text-white text-xs">ให้บริการเสร็จสิ้น</button>}{tab==="PENDING"&&<button onClick={()=>cancel(b.id)} className="px-3 py-1.5 rounded-lg border text-xs">ยกเลิก</button>}</div></div>
            </article>
          ))}
        </div>
      </section>
